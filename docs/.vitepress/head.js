// 头部文件
module.exports = [
    ['link', { rel: 'icon', type: 'image/svg+xml', href: '/images/hero.svg' }],
    ['meta', { name: 'keywords', content: '小工具,ai,openai中转,stable Diffusion,ChatGPT,ai绘画,novelai,sd,AI,Stable Diffusion,模型,作品' }],
    ['meta', { name:'author', content:'aueal.com'}],
    ['meta', { name:'author-website', content:'//www.aueal.com'}],
    ['meta', { name:'Copyright', content:'Aueal'}],
    ['link', { rel:'stylesheet', href:'//at.alicdn.com/t/c/font_4467349_cbzzdzuuu1.css'}],    
    ['link', { rel:'prefetch', href:'//at.alicdn.com/t/c/font_4467349_cbzzdzuuu1.js'}],
]